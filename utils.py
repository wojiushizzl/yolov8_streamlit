...
def _display_detected_frames(conf, model, st_frame, st_frame2, image):
    """
...
def infer_uploaded_webcam_cls(conf, model):
    """
    Execute inference for webcam.
    :param conf: Confidence of YOLOv8 model
    :param model: An instance of the `YOLOv8` class containing the YOLOv8 model.
    ...
    def video_frame_callback(frame):
        ...
        # Predict the objects in the image using YOLOv8 model
        res = model.predict(image, conf=conf)
        # print(res[0].probs.data[1]) #探测到分类1的概率
        
        # res_plotted = res[0].plot()
        res_plotted = image
        if res[0].probs.data[1] > conf:         
            color = [0, 0, 255]
            result='Missing Part!'
        else:
            color = [255, 0, 0]
...
def infer_uploaded_webcam_det_count(conf, model):
    """
    Execute inference for webcam.
    :param conf: Confidence of YOLOv8 model
    :param model: An instance of the `YOLOv8` class containing the YOLOv8 model.
...
def infer_uploaded_webcam_det_marking(conf, model):
    """
    Execute inference for webcam.
    :param conf: Confidence of YOLOv8 model
    :param model: An instance of the `YOLOv8` class containing the YOLOv8 model.
